      ...
      <div className="project-frame" id="top">
        ...
        <section className="workbench">
          <div className="scenario-head">
            ...
            <div className="run-actions"><button className="ghost" onClick={resetScenario}>Reset</button><button className="run-button" onClick={runScenario} disabled={calling || running}><span>{calling || running ? "●" : "▶"}</span>{calling ? "Calling n8n" : running ? "Rendering result" : "Run live scenario"}</button></div>
          </div>

          <section className="outcome-strip"><span>BUSINESS OUTCOME</span><p>{executionError || scenario.outcome}</p><div className={executionError ? "result error" : runStep === scenario.steps.length - 1 ? "result complete" : "result"}>{executionError ? "× n8n unavailable" : runStep === scenario.steps.length - 1 ? "✓ Live run completed" : calling ? "● Calling n8n" : running ? "● Rendering trace" : "○ Not executed"}</div></section>
